...
def setup_wpt_tracking(x0,mode):
    ...
    ocp.model.cost_y_expr = vertcat(model.x[0], 
                                    ...
                                    model.x[2], 
                                    model.x[3], 
                                    np.sqrt(model.x[3]**2+model.x[4]**2), 
                                    model.x[5], 
                                    model.x[6], 
                                    model.x[7], 
                                    model.u)
    ...
    x4d = model.f_expl_expr[4]*ship_p.dt
    x0_n = x0+x0d
    x1_n = x1+x1d
    x2_n = x2+x2d
    x3_n = x3+x3d
    ...
    if ship_p.CBF == 3:
        ...
        for i in range(5):
            ...
            B1 = np.sqrt( (ox_n-x0_n-R*cos(x2_n-np.arctan(x4_n/x3_n)-np.pi/2))**2 + (oy_n-x1_n-R*sin(x2_n-np.arctan(x4_n/x3_n)-np.pi/2))**2) - (obr+R)
            B2 = np.sqrt( (ox_n-x0_n-R*cos(x2_n-np.arctan(x4_n/x3_n)+np.pi/2))**2 + (oy_n-x1_n-R*sin(x2_n-np.arctan(x4_n/x3_n)+np.pi/2))**2) - (obr+R)
            if ship_p.TCCBF == 1:
                hkn = B1
            if ship_p.TCCBF == 2:
                hkn = B2
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-ship_p.dFxmax,-ship_p.dFxmax])
    ocp.constraints.ubu = np.array([+ship_p.dFxmax,+ship_p.dFxmax])
    ocp.constraints.idxbu = np.array([0, 1])

    ocp.constraints.lbx = np.array([ship_p.vmin, -ship_p.rmax, ship_p.Fxmin, ship_p.Fxmin])
    ocp.constraints.ubx = np.array([ship_p.vmax, ship_p.rmax, ship_p.Fxmax, ship_p.Fxmax])
