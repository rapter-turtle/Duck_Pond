...
from kinematic_model import export_kinematic_model
import scipy.linalg
import numpy as np
...
def setup_wpt_tracking(x0,mode):
    ...
    ocp.model.cost_y_expr = vertcat(model.x,
                                    model.u)
    ...
    x2 = model.x[2]
    x3 = model.x[3]
    x0d = model.f_expl_expr[0]*vehicle_p.dt
    x1d = model.f_expl_expr[1]*vehicle_p.dt
    x2d = model.f_expl_expr[2]*vehicle_p.dt
    x3d = model.f_expl_expr[3]*vehicle_p.dt
    x0_n = x0+x0d
    x1_n = x1+x1d
    x2_n = x2+x2d
    x3_n = x3+x3d
    ...
    if vehicle_p.CBF == 1:
        ...
        for i in range(5):
            ...
            # hk = Bdot + vehicle_p.gamma1/(obr+0.0001)*B
            hk = Bdot + vehicle_p.gamma1*B
            ...
            # hkn = Bdot + vehicle_p.gamma1/(obr+0.0001)*B
            hkn = Bdot + vehicle_p.gamma1*B
            h_expr[i] = hkn - (1-vehicle_p.gamma2)*hk
    ...
    # set constraints
    ocp.constraints.lbu = np.array([-vehicle_p.drotmax,-vehicle_p.daccmax])
    ocp.constraints.ubu = np.array([+vehicle_p.drotmax,+vehicle_p.daccmax])
    ocp.constraints.idxbu = np.array([0, 1])

    ocp.constraints.lbx = np.array([vehicle_p.vmin, -vehicle_p.rmax, vehicle_p.accmin])
    ocp.constraints.ubx = np.array([vehicle_p.vmax, vehicle_p.rmax, vehicle_p.accmax])
    ...
    # ocp.solver_options.qp_solver_cond_N = int(vehicle_p.N)

    # set prediction horizon
    ocp.solver_options.tf = int(vehicle_p.dt*vehicle_p.N)
