...
def L1_control(state, state_estim, param_filtered, dt, param_estim):
    ...
    f_usv = np.array([(- Xu*u - Xuu*u*u )/M,
          (-Yv*v - Yvv*v*v - Yr*r)/M,
          (- Nr*r - Nrrr*r*r - Nv*v)/I
          ])

    virtual_state = np.array([u*np.cos(psi) - v*np.sin(psi) - r*head_dist*np.sin(psi),
                              u*np.sin(psi) + v*np.cos(psi) + r*head_dist*np.cos(psi)])

    virtual_control = np.array([f_usv[0]*np.cos(psi) - u*r*np.sin(psi) - f_usv[1]*np.sin(psi) - v*r*np.cos(psi) - f_usv[2]*head_dist*np.sin(psi) - r*r*head_dist*np.cos(psi),
                                f_usv[0]*np.sin(psi) + u*r*np.cos(psi) + f_usv[1]*np.cos(psi) - v*r*np.sin(psi) + f_usv[2]*head_dist*np.cos(psi) - r*r*head_dist*np.sin(psi)])

    x_error = state_estim - virtual_state 

    xdot = param_estim + param_filtered - x_error 
