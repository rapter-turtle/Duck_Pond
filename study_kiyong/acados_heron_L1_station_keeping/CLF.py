...
def CLF_QP(state,param, weight, lamda):
    ...
    def head_ineq_constraint(x, state, param):
        ...
        V = (head_xn)*(head_xn) + (head_yn)*(head_yn) 
        V_dot = 2*(head_xn)*head_xn_dot + 2*(head_yn)*head_yn_dot
        V_dotdot =  2*(head_xn)*head_xn_dotdot + 2*(head_yn)*head_yn_dotdot + 2*head_xn_dot**2 + 2*head_yn_dot**2

        print("Head : ",np.sqrt(V))
    ...
    # Define the bounds
    bounds = [(-50, 50), (-50, 50), (None, None)]  # Bounds for x0, x1, and x2
