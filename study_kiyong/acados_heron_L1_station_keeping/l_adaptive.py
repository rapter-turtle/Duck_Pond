...
def L1_control(state, state_estim, param_filtered, dt, param_estim, MPC_control):
    ...
    nominal_control = np.array([((n1+n2)/M)*np.cos(psi)- head_dist*((-n1+n2)*dist/I)*np.sin(psi),
                                ((n1+n2)/M)*np.sin(psi)+ head_dist*((-n1+n2)*dist/I)*np.cos(psi)])

    state_feedback = np.array([virtual_state[0],
                                virtual_state[1]])
    
    state_feedback_estim = np.array([state_estim[0],
                               state_estim[1]])

    x_error = state_estim - virtual_state 

    
    adaptive_control = param_filtered - virtual_control + nominal_control #- state_feedback
    ...
    xdot = param_estim + param_filtered + nominal_control #- state_feedback
