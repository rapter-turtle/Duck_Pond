...
def CBF_QP(state, control_input, param, weight, ll, param_estim):
    ...
    def head_ineq_constraint(x, state, param, param_estim):
        ...
        disturbance_bound = abs(param[0]*10*np.cos(psi)) + abs(param[0]*10*np.sin(psi)) + abs(param[0]*10*head_dist*np.sin(psi)) + abs(param[1]*10*np.sin(psi)) + abs(param[1]*10*np.cos(psi)) + abs(param[1]*10*head_dist*np.cos(psi))


        # print(V_dotdot + (ll[0] + ll[1])*V_dot + ll[0]*ll[1]*V)
        return (V_dotdot + (ll[0] + ll[1])*V_dot + ll[0]*ll[1]*V + disturbance_bound + x[2])# Example inequality constraint
