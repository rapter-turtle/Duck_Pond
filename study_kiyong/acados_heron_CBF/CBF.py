...
def CBF_QP(state, control_input, param, weight, lamda):
    ...
    def example_ineq_constraint(x, state, param):
        ...
        V = param[0]*xn + param[1]*yn + param[2]
        V_dot = param[0]*(u*np.cos(psi) - v*np.sin(psi)) + param[1]*(u*np.sin(psi) + v*np.cos(psi))
        V_dotdot = param[0]*(u_dot*np.cos(psi) - u*r*np.sin(psi) - v_dot*np.sin(psi) - v*r*np.cos(psi)) + param[1]*(u_dot*np.sin(psi) + u*r*np.cos(psi) + v_dot*np.cos(psi) - v*r*np.sin(psi))
    ...
    ineq_constraints = [example_ineq_constraint]
