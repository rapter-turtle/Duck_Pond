...
class SensorFusionEKF:
    def __init__(self):
        ...
        traj_offset = np.array([353152, 4026032])
        trajectory_data =  np.load('ref_data_l1.npy')
        map_traj_data = np.empty_like(trajectory_data)        
        for i in range(trajectory_data.shape[0]):
            pos_x, pos_y = trajectory_data[i, 0] + traj_offset[0], trajectory_data[i, 1] + traj_offset[1]
