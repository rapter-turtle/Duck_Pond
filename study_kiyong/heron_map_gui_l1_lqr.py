...
class SensorFusionEKF:
    def __init__(self):
        ...
        self.vx = 0.0
        self.vy = 0.0
        self.station_keeping_x = 0.0
        self.station_keeping_y = 0.0
        self.R = 0.0
        self.omega = 0.0
        ...
        self.line1_m, = self.ax1.plot([], [], 'r-')    
        # self.refs, = self.ax1.plot(map_traj_data[:,0],map_traj_data[:,1],'b--',label='reference traj.')
        self.line1, = self.ax1.plot([], [], 'k-')                      
        self.pred_line, = self.ax1.plot([], [], 'm-')
        self.ref_line, = self.ax1.plot([], [], 'm-')        
        self.line1test, = self.ax1.plot([], [], 'm.')   

        ### desired traj
        self.line_wpt, = self.ax1.plot([], [], 'g-',label='wpt')  
        self.circle = Circle((0, 0), radius=1, color='g', fill=False, label='circle')
        ...
        self.traj_sub = rospy.Subscriber('/l1_data', Float64MultiArray, self.traj_callback)
        # rospy.Publisher('/l1_data', Float64MultiArray)
    ...
    def traj_callback(self, msg):# - 주기가 gps callback 주기랑 같음 - gps data callback받으면 ekf에서 publish 하기때문
        # # Straght
        # self.vx = msg.data[10]
        # self.vy = msg.data[11]
        # self.station_keeping_x = msg.data[8]
        # self.station_keeping_y = msg.data[9]
        
        # Circling
        self.R = msg.data[10]
        self.omega = msg.data[11]
        self.station_keeping_x = msg.data[8]
        self.station_keeping_y = msg.data[9]        
    ...
    def update_plot(self, frame):
        if len(self.time_data)>1:
            ...
            # print(wpt_kh_list)
            # llll = np.array([[600.15193456, 250.39277559],
            # [589.6413928 , 245.15092381]])
            ## L1 trajectory straight
            a1, a2 = self.utm_to_map(self.station_keeping_x, self.station_keeping_y)
            b1, b2 = self.utm_to_map(self.station_keeping_x + 100*self.vx, self.station_keeping_y + 100*self.vy)
            llll = np.array([[a1, a2],
            [b1, b2]])
            self.line_wpt.set_data(llll[:,0],llll[:,1])
            
            ## L1 trajectory circle
            circle_center_x, circle_center_y = self.utm_to_map(self.station_keeping_x, self.station_keeping_y)
            self.circle.center = (circle_center_x, circle_center_y)
            self.circle.set_radius(self.R)  # Update the radius based on the R parameter
