...
class HeronMPC:
    def __init__(self):
        ...
        self.amplitude=2.0  # amplitude of the sine wave
        ...
        self.Q_weight = [15, 15, 1e-3, 2, 0, 1e-3, 1e-4, 1e-4]
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            for j in range(self.N+1):
                ...
                yref = np.hstack((refs[0],refs[1],refs[2],self.velocity,0, 0,0,0,0,0))
                # print(refs)
                if j == self.N:
                    yref = np.hstack((refs[0],refs[1],refs[2],self.velocity,0,0,0,0))
            ...
            st, ps, pf, th = L1_control(self.states, self.state_estim, self.param_filtered, self.con_dt, self.param_estim, self.cutoff_frequency)
            self.state_estim, self.param_estim, self.param_filtered, thruster = st, ps, pf, th
            ...
            drive_msg = Drive()
            drive_msg.left = self.force_to_heron_command(thruster[0] + self.n1,1)
            drive_msg.right = self.force_to_heron_command(thruster[1] + self.n2,2)
