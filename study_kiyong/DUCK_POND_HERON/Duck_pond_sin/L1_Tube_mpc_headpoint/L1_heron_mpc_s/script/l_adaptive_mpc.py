...
def L1_control(state, state_estim, param_filtered, dt, param_estim, w_cutoff, feedback_con, feedback):
    ...
    f_usv = np.array([((n1 + n2) + (n1_f + n2_f) - (Xu + Xuu * np.sqrt(u * u)) * u) / M,
          ( -Yv * v - Yvv * np.sqrt(v * v ) * v - Yr * r) / M,
          ((-n1 + n2 - n1_f + n2_f) * dist - (Nr + Nrrr * r * r) * r - Nv * v) / I
          ])
    ...
    # adaptive_control = np.array([0.0,0.0])
