...
class HeronMPC:
    def __init__(self):
        ...
        self.head_dist = 1.0
        self.nominal_states = np.zeros(8)
        self.nominal_states[0] = self.head_dist*np.cos(self.p)
        self.nominal_states[1] = self.head_dist*np.sin(self.p)        
        ...
        self.tfinal=10000.0  # total time duration for the trajectory
        # self.dt=self.ref_dt  # time step for the trajectory
        self.translation=(10, -5)  # translation to apply to the trajectory (tuple of x, y)
        self.theta=0.4*np.pi + np.pi/2.5#np.pi/2  # rotation angle in radians (pi/2 for 90 degrees)
        self.num_s_shapes=10.0  # number of S-shapes (sine wave cycles)
        self.start_point=(15, -5)#(-5, -0)  # starting point of the trajectory
        self.amplitude=1.6  # amplitude of the sine wave
        self.wavelength=20.0  # wavelength of the sine wave
        self.velocity = 0.7
        self.straight_length = 15.0
        ...
        self.Q_weight = [3, 3, 0, 0, 0, 0, 1e-4, 1e-4]
        self.R_weight = [1e-3, 1e-3]
         
        # L1 vairable
        self.param_estim = np.array([0.0, 0.0])
        self.param_filtered = np.array([0.0, 0.0])
        self.state_estim = np.array([0.0, 0.0])
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            obs_pos = np.array([self.x+100-offset[0], self.y+100-offset[1], 1,  # Obstacle-1: x, y, radius
                                self.x+100-offset[0], self.y+100-offset[1], 1]) # Obstacle-2: x, y, radius
            for j in range(self.N+1):
            ...
            self.ocp_solver.set(0, "lbx", self.nominal_states)
            self.ocp_solver.set(0, "ubx", self.nominal_states)
            ...
            ns, feedbak_con, feedback = nom_sym(self.states, self.con_dt, self.nominal_states)
            st, ps, pf, th = L1_control(self.states, self.state_estim, self.param_filtered, self.con_dt, self.param_estim, self.cutoff_frequency, feedbak_con, feedback)
            self.state_estim, self.param_estim, self.param_filtered, thruster, self.nominal_states = st, ps, pf, th, ns

            self.nominal_states[6] = self.n1
            self.nominal_states[7] = self.n2

            # print("nom : ", self.nominal_states)
            # print("real : ", self.HG)
            ...
            drive_msg = Drive()
            drive_msg.left = self.force_to_heron_command(thruster[0] + feedbak_con[0] + self.n1, 1)
            drive_msg.right = self.force_to_heron_command(thruster[1]+ feedbak_con[1] + self.n2, 2)
            ...
            traj_data = [self.tfinal, self.ref_dt, self.theta, self.num_s_shapes, self.start_point[0], self.start_point[1], self.amplitude, self.wavelength, self.velocity, offset[0], offset[1], self.straight_length,self.translation[0],self.translation[1]]
            ...
            L1_data = [self.param_filtered[0], self.param_filtered[1], self.param_estim[0], self.param_estim[1], self.states[7], self.states[6], thruster[1]+self.n2, thruster[0]+self.n1]
