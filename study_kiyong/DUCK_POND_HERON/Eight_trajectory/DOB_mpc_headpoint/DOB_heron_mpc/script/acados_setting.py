...
def export_heron_model() -> AcadosModel:
    ...
    u_disturbance  = SX.sym('u_disturbance')
    v_disturbance  = SX.sym('v_disturbance')
    r_disturbance  = SX.sym('r_disturbance')
    ...
    p = vertcat(ox1, oy1, or1, 
                ox2, oy2, or2, u_disturbance, v_disturbance, r_disturbance)
    ...
    f_expl = vertcat(u*cos(psi) - v*sin(psi) - head_dist*r*sin(psi),
                     ...
                     r,
                     ( (n1+n2) - Xu*u - Xuu*sqrt(u*u+eps)*u + u_disturbance)/M ,
                     ( -Yv*v - Yvv*sqrt(v*v+eps)*v - Yr*r + v_disturbance)/M ,
                     ( (-n1+n2)*dist - Nr*r - Nrrr*r*r*r - Nv*v+ r_disturbance)/I ,
                     n1d,
                     n2d
                     )
...
def setup_trajectory_tracking(x0, N_horizon, Tf):
    ...
    h_expr[0] = 1.0#(model.x[0]-ox1) ** 2 + (model.x[1] - oy1) ** 2 - or1**2
    h_expr[1] = 1.0#(model.x[0]-ox2) ** 2 + (model.x[1] - oy2) ** 2 - or2**2
    ...
    # ocp.constraints.lbx = np.array([-8, -8])
    # ocp.constraints.ubx = np.array([25, 25])
    ocp.constraints.lbx = np.array([-2.0, -2.0])
    ocp.constraints.ubx = np.array([22, 22])
