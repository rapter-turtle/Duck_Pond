...
def export_heron_model() -> AcadosModel:
    ...
    states = vertcat(xn, yn, Vx, Vy, nx, ny)

    nxd  = SX.sym('nxd')
    nyd  = SX.sym('nyd')
    inputs   = vertcat(nxd, nyd)
    ...
    nx_dot   = SX.sym('nx_dot')
    ny_dot   = SX.sym('ny_dot')

    states_dot = vertcat(xn_dot, yn_dot, Vx_dot, Vy_dot, nx_dot, ny_dot)
...
def setup_trajectory_tracking(x0, N_horizon, Tf):
    ...
    Q_mat = 2*np.diag([1, 1, 5, 5, 1e-3, 1e-3])
