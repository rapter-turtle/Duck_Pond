...
class HeronMPC:
    def __init__(self):
        ...
        self.Q_weight = [2, 2, 0, 1e-1, 1e-1, 1e-6, 1e-4, 1e-4]
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            self.state_estim, self.param_estim, self.param_filtered, thruster = L1_control(self.states, self.state_estim, self.param_filtered, self.con_dt, self.param_estim, self.cutoff_frequency)
            ...
            drive_msg = Drive()
            drive_msg.right = self.force_to_heron_command(thruster[0],1)
            drive_msg.left = self.force_to_heron_command(thruster[1],2)
