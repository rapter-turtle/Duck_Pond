...
disturbance = 2 * sin(0.3*t); % Disturbance (example sinusoidal disturbance)
...
for k = 1:n-1
    ...
    % u(k) = -c1*x2(k) - z2(k) - integ(k)*k3 - lambda1*sqrt(abs(s_hat(k))) * sign(s_hat(k)) - lambda2 * integ_s(k);


    % System dynamics
    x1(k+1) = x1(k) + dt * x2(k);
    x2(k+1) = x2(k) + dt * (u(k) + disturbance(k));
