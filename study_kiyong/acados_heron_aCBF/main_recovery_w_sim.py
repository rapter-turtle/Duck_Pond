...
def main():
    ...
    for i in range(int(T_final/simulation_dt)):
        if i%int(con_dt/simulation_dt) == 0:
            ...
            for j in range(N_horizon):
                yref = np.array([x_tship[0]+v_x_tship*j*con_dt - 2.5*np.cos(x_tship[2] + 3.141592*0.5), 
                                ...
                                x_tship[2], v_x_tship, v_y_tship, 0, 0, 0, 0, 0])
            ...
            yref_N = np.array([x_tship[0]+v_x_tship*N_horizon*con_dt- 2.5*np.cos(x_tship[2] + 3.141592*0.5), 
                            ...
                            x_tship[2], v_x_tship, v_y_tship, 0, 0, 0])
            ocp_solver.cost_set(N_horizon, "yref", yref_N)
        ...
        ##### L1 adaptive #####   
        state_estim, param_estim, thrust = DOB_CBF(simX[i, :], state_estim, simulation_dt, param_estim)
        ...
        disturbance_state[:3], XY_wave_force = wave_disturbance(disturbance_state[:3], wind_direction, wind_speed, 0.1, 0.1, 0.3, 2, 0.1, simulation_dt)
        ...
        V = oa*(simX[i,0] + head_dist*np.cos(psi)) + ob*(simX[i,1] + head_dist*np.sin(psi)) + oc
        V0 = oa*(simX[0,0] + head_dist*np.cos(simX[0,2])) + ob*(simX[0,1] + head_dist*np.sin(simX[0,2])) + oc
