...
def main():
    ...
    param_estim_dob = np.zeros((2))
    # param_filtered = np.zeros((2))
    state_estim_dob = np.array([0.0, 2.0, 0.0 , 1, 0.0])
    ...
    dob_save = np.zeros((int(T_final/simulation_dt)+1, 8))
    ...
    for i in range(int(T_final/simulation_dt)):
        ...
        ##### DOB #####
        # state_estim_dob, param_estim_dob = Disturbance_observer(simX[i, :], state_estim_dob, simulation_dt, param_estim_dob)
        # M = 36 # Mass [kg]
        # I = 8.35 # Inertial tensor [kg m^2]
        # L = 0.73 # length [m]
        # disturbance_u = param_estim_dob[0]
        # disturbance_r = param_estim_dob[1]
        # l1_control[0] = -(M*param_estim_dob[0] - 2*(I/L)*param_estim_dob[1])*0.5
        # l1_control[1] = -(M*param_estim_dob[0] + 2*(I/L)*param_estim_dob[1])*0.5
        # # DOB[0] = -(M*param_estim_dob[0] - 2*(I/L)*param_estim_dob[1])*0.5
        # # DOB[1] = -(M*param_estim_dob[0] + 2*(I/L)*param_estim_dob[1])*0.5        
        # # print(param_estim)
        ##### DOB #####
        ...
        dob_save[i,:] = np.hstack((disturbance, l1_control, L1_XN, DOB))
