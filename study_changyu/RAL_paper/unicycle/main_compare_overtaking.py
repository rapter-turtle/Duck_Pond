...
if __name__ == '__main__':
    ...
    cd2, at2 ,sv2 ,con2, simX2, simU2, target_speed2, obs_array2, cbf_and_dist2, time_save2  = main(1,'overtaking', 10, 0.5, 0.05, target_speed)
    # cd3, at3 ,sv3 ,con3, simX3, simU3, target_speed3, obs_array3, cbf_and_dist3, time_save3  = main(1,'overtaking', 10, 0.50, 0.05, target_speed)
    # cd4, at4 ,sv4 ,con4, simX4, simU4, target_speed4, obs_array4, cbf_and_dist4, time_save4  = main(1,'overtaking', 10, 0.25, 0.01, target_speed)
    # cd5, at5 ,sv5 ,con5, simX5, simU5, target_speed5, obs_array5, cbf_and_dist5, time_save5  = main(1,'overtaking', 10, 0.25, 0.10, target_speed)
    cd6, at6 ,sv6 ,con6, simX6, simU6, target_speed6, obs_array6, cbf_and_dist6, time_save6  = main(2,'overtaking', 10, 1, 0.05, target_speed)
    ...
    if plot_results:
            ...
            for i in range(len(simX)):
                if i%25 == 0:
                    ...
                    if idx == 1 and i > 0:
                        label = f'{i / 10:.1f} s'
                        if i < 160:
                            ax_asv.annotate(label, (position[0], position[1]), textcoords="offset points", xytext=(0, -15), ha='center', fontsize=FS-5, color='black')
                        else:
        ...
        ax_asv.annotate('0.0 s', (obs_array[0][0], obs_array[0][1]), textcoords="offset points", xytext=(0, 25), ha='center', fontsize=FS-5, color='black')
        ax_asv.annotate('20.0 s', (obs_array[-1][0], obs_array[-1][1]), textcoords="offset points", xytext=(-19, 25), ha='center', fontsize=FS-5, color='black')
        # Draw an arrow between the two annotated points
        start_position = obs_array[0][0:2] + [1.0,1.65]
        end_position = obs_array[-1][0:2] + [-2.3,1.65]
        ...
        ax_asv.set(xlim=(0, target_x),ylim=(-4,2.5))
        # ax_asv.grid(True)
    ...
    if plot_box:
        ...
        data = {
            ...
            "closest_dist": [cd2,cd6],
            "arrival_time":[ at2, at6],
            "speed_var":  [sv2,   sv6],
            "con_usage":  [con2,   con6],
        }
